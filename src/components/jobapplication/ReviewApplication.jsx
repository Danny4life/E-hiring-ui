...
import ApplicationProfileUpdate from "./ApplicationProfileUpdate";

const ReviewApplication = ({ apply, userData, selectedJobPost }) => {
  ...
  const handleApplyForJob = async (e) => {
    ...
    try {
      await axios
        .post("http://localhost:8080/job-seeker/submit-application", formData, {
          headers: {
    ...
    <div className="absolute my-auto justify-end items-end rounded bg-white flex flex-col px-6 max-md:px-5 max-w-[528px]">
      ...
      {!showInfo && (
        <form
          ...
          className="self-start rounded bg-white flex flex-col max-md:px-5 max-w-[528px]"
        >
          ...
          <div className="flex justify-between items-center">
            <button
              onClick={apply}
              className="text-white text-base font-semibold leading-6 tracking-normal whitespace-nowrap flex justify-center items-stretch bg-blue-500 w-fit max-w-full my-6 px-4 py-2 rounded-xl self-end cursor-pointer"
            >
              Cancel
            </button>
