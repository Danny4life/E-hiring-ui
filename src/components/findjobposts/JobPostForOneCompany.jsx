...
const JobPostForOneCompany = ({ companyName }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await axios
        .post("http://localhost:8080/job-seeker/employer", formData, {
          headers: {
  ...
  return (
    <div className="items-stretch flex flex-col mb-15 max-w-[1000px] mx-auto">
      <Link
        to={"/jobseeker-page"}
        className="sticky top-[1rem] cursor-pointer justify-between rounded-xl bg-emerald-100 flex gap-2 px-2.5 py-4 items-start"
      >
      ...
      <div className="mt-[7rem] text-zinc-500 text-sm leading-5 tracking-normal w-full mt-6 max-md:max-w-full px-5">
        There are {details?.length} {details[0]?.companyName} jobs available.
        You can check out other available jobs below.
      </div>
...
export default JobPostForOneCompany;
