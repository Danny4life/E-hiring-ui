...
import ReportDetails from "../utils/ReportDetails";

const JobPostHalfDisplay = ({
  handleSeeMore,
  handleFindJobsOneCompany,
  seeMore,
  selectedJobPost,
  apply,
  handleBackDrop,
  ...
  return (
    <div
      className="sticky top-[6rem] min-w-[43vw] items-start flex flex-col max-md:max-w-full max-md:mt-10 max-w-[1000px] mx-auto"
      style={{ width: `${!seeMore ? "43vw" : "70vw"}` }}
    >
      <div className="items-stretch border-[color:var(--Gray-3,#828282)] self-stretch flex flex-col p-6 rounded-xl border-[0.5px] border-solid max-md:max-w-full max-md:px-5">
        ...
        <div
          onClick={handleFindJobsOneCompany}
          className="hover:text-blue-500 cursor-pointer text-black text-base leading-6 tracking-normal whitespace-nowrap mt-3 self-start"
        >
        ...
        <div className="text-blue-500 text-lg font-medium leading-6 tracking-normal whitespace-nowrap mt-3 max-md:max-w-full">
          {`₦${selectedJobPost.minimumPay} - ₦${selectedJobPost.maximumPay} / ${
            selectedJobPost.payRate === "PER_HOUR"
              ? "Per Hour"
              : selectedJobPost.payRate === "PER_WEEK"
              ? "Per Week"
              : selectedJobPost.payRate === "PER_MONTH"
              ? "Per Month"
              : selectedJobPost.payRate === "PER_YEAR"
              ? "Per Year"
              : ""
          }`}
        </div>
        ...
        <button
          onClick={apply}
          className="text-white text-base font-semibold leading-6 tracking-normal whitespace-nowrap bg-blue-500 w-[115px] max-w-full mt-3 px-4 py-2 rounded-xl self-start"
        >
          Apply now
        </button>
      </div>
      ...
      <div className="px-4 text-black text-base font-medium leading-6 tracking-normal mt-3.5 max-md:max-w-full">
        <ul>
          {selectedJobPost.responsibilities.map((responsibility) => (
      ...
      {seeMore && (
        <>
          ...
          <div className="px-4 text-black text-lg font-medium leading-6 tracking-normal w-full mt-3.5 max-md:max-w-full">
            <ul>
              {selectedJobPost.qualifications.map((qualification) => (
          ...
          <div className="px-4 text-black text-lg font-medium leading-6 tracking-normal w-full mt-3.5 max-md:max-w-full">
            <ul>
              {selectedJobPost.niceToHave.map((nice2have) => (
      ...
      <div
        ...
        className="cursor-pointer text-blue-500 text-base font-medium leading-6 tracking-normal whitespace-nowrap mt-2 mb-10 self-end"
      >
        {" "}
...
export default JobPostHalfDisplay;
