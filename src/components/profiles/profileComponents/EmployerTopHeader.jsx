...
const EmployerTopHeader = ({
  ...
  return (
    <div
      className="sticky top-[0] shadow-lg justify-between items-stretch bg-white flex w-full gap-5 px-12 pt-4 max-md:max-w-full max-md:flex-wrap max-md:px-5"
      style={{ zIndex: 3 }}
    >
      <div className="text-blue-500 text-xl leading-7 tracking-normal my-auto">
        <img
          className="w-14/14 h-20 cursor-pointer"
          src={swiftLogo}
          alt="Company Logo"
        />
      </div>
      ...
      <div className="self-center flex gap-6 my-auto">
        <div
          onClick={handleFindCandidatePage}
          className="group relative cursor-pointer"
        >
          <div
            onClick={activePageToFindJobs}
            className={`${
              active === "findCandidates" ? "active-nav" : ""
            } text-black-200 text-base font-bold leading-6 tracking-normal transition-colors duration-300 group-hover:text-blue-500 whitespace-nowrap`}
          >
            Find Candidates
          </div>
      ...
      <div className="items-center flex justify-between gap-5 max-md:justify-center">
        ...
        <button
          onClick={handlePostJobPage}
          className="text-white text-base font-semibold leading-6 tracking-normal whitespace-nowrap justify-center items-stretch bg-blue-500 px-4 h-fit py-2 rounded-xl"
        >
          Post New Job
        </button>
      </div>
      ...
      {profileClick && (
        <div className="absolute right-[2rem] top-[5.4rem] shadow-2xl">
          <ProfilePopUp
            ...
            handleProfile={activePageToProfile}
          />
        </div>
      )}
    </div>
