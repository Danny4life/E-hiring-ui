...
const CoverLetter = () => {
  ...
  function uploadSingleFile(coverletter) {
    ...
    axios
      .put("http://localhost:8080/job-seeker/update-profile/cover-letter", formData, {
        headers: {
      ...
      .then((result) => {
        ...
        SweetAlert(
          "success",
          "Update Successful",
          "coverletter uploaded successfully"
        );
      })
      .catch((error) => {
        SweetAlert(
  ...
  return (
    ...
    >
      <div className="items-start flex grow flex-col max-md:max-w-full max-md:mt-10">
        ...
        <div
          {...getRootProps()}
          className="w-[30vw] max-h-100 border-2 border-dashed border-gray-300 rounded-lg p-4 mb-4 mt-4 text-white text-center"
        >
          <input {...getInputProps()} />
