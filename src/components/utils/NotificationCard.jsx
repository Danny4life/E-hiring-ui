...
const NotificationCard = () => {
  ...
  const handleReadNotification = async (id) => {
    ...
    try {
      await axios
        .patch(`http://localhost:8080/notification/mark-as-read`, formData, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        })
        ...
        {notifications.length !== 0 ? (
          notifications.map((notification) => (
            <div
              ...
              className={`${
                notification.read ? "" : "bg-green-500"
              } pr-2 cursor-pointer grow justify-center flex items-center gap-8 mt-2 max-md:mt-10`}
