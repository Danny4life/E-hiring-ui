...
const ReportDetails = ({ handleReportDetails, selectedJobPost }) => {
  ...
  const handleReport = async (e) => {
    ...
    try {
      await axios
        .post("http://localhost:8080/job-seeker/report", formData, {
          headers: {
      ...
      <div className="absolute right-[17.5vw] flex justify-center">
        <div className="absolute justify-center items-center bg-gray-200 flex flex-col gap-3 px-8 rounded-2xl max-md:px-5 max-w-[33rem] max-h-[26rem]">
          ...
          >
            ...
            <option value="JOB_IS_NO_LONGER_AVAILABLE">
              Job is no longer available
            </option>
            <option value="ADVERTISER_ASKED_FOR_MONEY">
              Advertiser asked for money
            </option>
          </select>
          ...
          <div className="flex justify-between w-full">
            <button
              ...
              className="h-fit text-gray-50 text-base font-semibold leading-4 whitespace-nowrap justify-center items-center bg-blue-500 self-stretch my-8 px-5 py-3 rounded-xl max-md:max-w-full"
            >
              Cancel
            </button>
